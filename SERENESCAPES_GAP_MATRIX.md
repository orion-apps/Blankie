...
| Library/download UX | Downloaded/available + server workflows | 🚧 Added Library status + refresh + searchable lists, plus real remote file download to local storage, persisted task states, progress/retry/remove UI, and local-file reconciliation | Remaining: integrate downloaded remote files into active playback selection and add true background download handling | Medium | Wire downloaded-remote playback path and background transfer/resume |
