...
| Library/download UX | Downloaded/available + server workflows | 🚧 Added Library status + refresh + searchable lists, real remote file download to local storage, persisted task states, progress/retry/remove UI, local-file reconciliation, downloaded-remote playback controls, interrupted-download recovery, background URLSession pipeline, app-level background session event wiring, and mixer visibility for downloaded remote tracks | Remaining: deep blend-level remote-track mixing/inclusion in preset state model | Medium | Extend preset/blend schema to include remote-track members and playback routing |
