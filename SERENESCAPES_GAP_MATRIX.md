...
| Library/download UX | Downloaded/available + server workflows | 🚧 Added Library status + refresh + searchable lists, real remote file download/local storage, persisted task states, progress/retry/remove UI, local-file reconciliation, downloaded-remote playback controls, background URLSession pipeline + app wiring, preset/blend schema inclusion for selected remote tracks, and per-remote volume/pan mixer controls | Remaining: full blend-card preview parity polish for multi-remote sets and deeper remote metadata UI polish | Medium | Polish multi-remote blend UX and finalize parity QA checklist |
