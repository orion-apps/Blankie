...
| Library/download UX | Downloaded/available + server workflows | 🚧 Added Library status + refresh + searchable lists, real remote file download to local storage, persisted task states, progress/retry/remove UI, local-file reconciliation, downloaded-remote playback controls, and interrupted-download recovery on relaunch | Remaining: true iOS background transfer session support (URLSession background config) and full remote-track integration into blend/mixer playback model | Medium | Add background URLSession download pipeline + blend-level remote integration |
