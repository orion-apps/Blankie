...
| Library/download UX | Downloaded/available + server workflows | 🚧 Added Library status + refresh + searchable lists, plus download task states (queued/downloading/completed/failed), progress bars, retry/remove actions, and persisted download status | Still missing real file-transfer backend integration (currently simulated state machine) | Medium | Wire real remote file download/storage pipeline and reconcile with task UI |
